...
bool ZCommand::reSaveConfig(int retries)
{
  ...
  f.printf("%d,%d,%d,%d,%d,",longResponses, 0, dcdMode, serialConfig, ctsMode);
...
void ZCommand::setOptionsFromSavedConfig(String configArguments[])
{
  ...
  if(configArguments[CFG_S64_DROPNULL].length()>0)
    dropNull = atoi(configArguments[CFG_S64_DROPNULL].c_str());
...
ZResult ZCommand::doInfoCommand(int vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  ...
  case 5:
  {
    ...
    if(dropNull || showAll)
      serial.prints(dropNull ? "S64=1" : "S64=0");
...
ZResult ZCommand::doEOLNCommand(int vval, uint8_t *vbuf, int vlen, bool isNumber)
{
  ...
  {
    String cmd="";
    for(int i=0;i<vlen;i++) cmd += (char)vbuf[i];
    cmd.toUpperCase();
    if((cmd=="D")||(cmd=="T"))
    {
      serial.prints(znistGetDateTimeUTC());
      serial.prints(EOLN);
      return ZOK;
    }
  }
...
ZResult ZCommand::doSerialCommand()
{
  ...
  &&(lc(sbuf[index+1])=='t'))
  {
    String __rest=sbuf.substring(index+2);
    __rest.toUpperCase();
    if((__rest=="RD")||(__rest=="RT"))
    {
      serial.prints(znistGetDateTimeUTC());
      serial.prints(EOLN);
