...
bool ZPrint::testPrinterSpec(const char *vbuf, int vlen)
{
...
ZResult ZPrint::switchTo(char *vbuf, int vlen)
{
  debugPrintf("\r\nMode:Print\r\n");
  char *workBuf = (char *)malloc(vlen+1);
  strcpy(workBuf, vbuf);
...
ZResult ZPrint::finishSwitchTo(char *hostIp, char *req, int port, bool doSSL, PrintPayloadType pType)
{
  ...
  chunkLen = buildIPPAttribute(pbuf, 0x42, "requesting-user-name", "perryzi", 7);
  writeChunk(pbuf, chunkLen);
  ...
  char jobName[12];
  sprintf(jobName, "perryzi-j%c%c", jobChar1, jobChar2);
